<x-layouts.main title="Ajouter mon établissement">
    ...
    <x-navbar/>
    <form action="{{route('shop.store')}}" method="POST" class="grid max-w-xl grid-cols-12 gap-4 p-4 px-4 mx-auto mb-20 bg-white rounded-md">
        @csrf
        <x-input.text :required="true" classes="col-span-12" label="Nom de votre établissement" name="name"/>
        <x-input.text :required="false" classes="col-span-12" label="Intitulé" name="title" placeholder="commerce, restaurant, producteur de fruits,..."/>
        <x-input.text classes="col-span-12" label="Adresse" name="street" placeholder="52, rue saint-martin"/>
        <x-input.text classes="col-span-12" label="Ville" name="logo" placeholder="Tournai"/>
        <x-input.file classes="col-span-12" label="Logo (Max 2mb)" name="city" />
        ...
        <x-input.text classes="col-span-6" label="Votre adresse email" name="email" placeholder="sample@gmail.com"/>
